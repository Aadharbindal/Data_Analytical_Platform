...
export default function DistributionExplorer() {
  const { distributions, isLoading } = useDatasetAnalytics("demo", "v1");

  if (isLoading) return <div className="p-8">Loading Distributions...</div>;

  const data = distributions.data;
  if (!data) return <div className="p-8">No Distribution data found.</div>;
